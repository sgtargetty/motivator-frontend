...
class NotificationManager {
  ...
  Future<void> requestAwesomeNotificationPermissions() async {
    ...
    if (isAllowed) {
      ...
      // 3. Request enhanced permissions for amber alerts
      try {
        final systemAlertPermission = await Permission.systemAlertWindow.status;
        final batteryOptimizationPermission = await Permission.ignoreBatteryOptimizations.status;
        
        if (systemAlertPermission != PermissionStatus.granted) {
          print('⚠️ System alert window permission not granted');
        }
        
        if (batteryOptimizationPermission != PermissionStatus.granted) {
          print('⚠️ Battery optimization permission not granted');
        }
        
      } catch (e) {
        print('⚠️ Enhanced permission check failed: $e');
      }
  ...
  static Future<void> _onNotificationActionReceived(ReceivedAction receivedAction) async {
    ...
    try {
      // 🚨 Handle emergency button press (EXACT HIJACK VERSION LOGIC)
      if (receivedAction.buttonKeyPressed == 'OPEN_EMERGENCY') {
        ...
        if (context != null) {
          ...
          // 🚨 BACKUP VIBRATION PATTERN (in case immediate vibration didn't work)
          try {
            for (int i = 0; i < 5; i++) {
              HapticFeedback.heavyImpact();
  ...
  Future<void> _triggerEmergencyVibrationPattern() async {
    ...
    try {
      ...
      print('❌ Error with emergency vibration pattern: $e');
    }
  }
  ...
  Future<void> _playEmergencyAudio(String audioFilePath, bool forceOverrideSilent) async {
    ...
    try {
      ...
      if (forceOverrideSilent) {
        print('🔊 Override silent mode requested');
      }
  ...
  // ===== MISSING METHODS FOR TASK SCHEDULER =====
  ...
  Future<void> createForcefulAmberAlert({
    ...
    try {
      ...
      await _triggerEmergencyVibrationPattern();
